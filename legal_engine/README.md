...
### 5. L_policy_gap (사법 개입 필요성)

> **레이어 성격**: L_policy_gap은 시장(법정) 상태가 아니라 **통치 개입 필요성**을 반영하는 행정 레이어다.
> 다른 4개 레이어가 법정 물리량(진실·증거·법리·편향)을 측정하는 반면,
> L_policy_gap은 "그 신호들에 대해 사법 개혁이 얼마나 미개입 상태인가"를 측정한다.
...
from legal_engine.legal_engine import LegalEngine
from legal_engine.pharaoh_decree_legal import JudicialEvent
...
> **pytest 실행**: `60_APPLIED_LAYER/` 에서 `python -m pytest legal_engine/tests/test_legal.py -v`
...
| 버전 | 날짜 | 내용 |
|------|------|------|
| **v0.1.1** | 2026-03-17 | README 동기화 — 시나리오 A/B 실제 실행 결과 갱신, ODE 수식 정오(dT 정치개입항·dE 변호공식), jury_compromised 플래그 weight 독립 계산, import 예시 수정 |
